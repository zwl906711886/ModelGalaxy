...
def computeCorrelDist(corrMats, outputName, from_folder, to_path):
	...
	counter = 0
	# print(corrMats.shape)
	for i in corrMats:
		print(corrMats[i].shape)
		numpy.savetxt(to_path + ".csv", corrMats[i], delimiter=",")
		pkl_path = from_folder + 'task_embbeding.pkl'
		with open(pkl_path,'ab') as file2:
			pickle.dump(corrMats[i],file2)
...
from_folder = "output24/gw/dataset"
to_path = from_folder + 'embed'
main(from_folder, to_path)
